...
export default function MyRecipe() {
  ...
  useEffect(() => {
    ...
    const fetchMyRecipes = async () => {
      try {
        ...
        const res = await fetch("https://enbla-recipe-sharing-app-16il.onrender.com/api/recipes/my", {
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`https://enbla-recipe-sharing-app-16il.onrender.com/api/recipes/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
